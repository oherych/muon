package muon

type G string

var (
	testString     = "test"
	testLongString = "test Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce mi mauris, fringilla a gravida ac, vulputate vitae dui. Proin rhoncus ante vitae purus mollis, id hendrerit tellus tempor. Aliquam ut ex nibh. Aenean quis quam eu purus scelerisque viverra ac consequat justo. Sed lobortis interdum facilisis. Sed euismod est magna, at iaculis nisi mollis a. Maecenas nec diam augue. Phasellus volutpat mattis nisi, eu sagittis enim tempor vitae. Aliquam sit amet ante finibus, bibendum lorem et, porta libero. Sed eu."

	tests = map[string]struct {
		golang  interface{}
		encoded []byte
		tokens  []Token
	}{
		"nil": {
			golang:  nil,
			encoded: []byte{nilValue},
			tokens:  []Token{{A: tokenNil}},
		},
		"string_empty": {
			golang:  "",
			encoded: []byte{stringEnd},
			tokens:  []Token{{A: TokenString, Data: ""}},
		},
		"string": {
			golang:  testString,
			encoded: []byte{0x74, 0x65, 0x73, 0x74, stringEnd},
			tokens:  []Token{{A: TokenString, Data: testString}},
		},
		"string_with_zero": {
			golang:  "te" + string([]byte{stringEnd}) + "st",
			encoded: []byte{stringStart, 0x5, 0x74, 0x65, 0x0, 0x73, 0x74},
			tokens:  []Token{{A: TokenString, Data: "te" + string([]byte{stringEnd}) + "st"}},
		},
		"long_string": {
			golang:  testLongString,
			encoded: []byte{stringStart, 0x86, 0x4, },
			tokens:  []Token{{A: TokenString, Data: testLongString}},
		},
		"kind_string": {
			golang:  G(testString),
			encoded: []byte{0x74, 0x65, 0x73, 0x74, stringEnd},
			tokens:  []Token{{A: TokenString, Data: testString}},
		},

		"pointer": {
			golang:  &testString,
			encoded: []byte{0x74, 0x65, 0x73, 0x74, stringEnd},
			tokens:  []Token{{A: TokenString, Data: testString}},
		},

		"true": {
			golang:  true,
			encoded: []byte{boolTrue},
			tokens:  []Token{{A: tokenTrue}},
		},
		"false": {
			golang:  false,
			encoded: []byte{boolFalse},
			tokens:  []Token{{A: tokenFalse}},
		},

		"int": {
			golang:  5,
			encoded: []byte{0xa5},
			tokens:  []Token{{A: TokenNumber, Data: 5}},
		},
		"int8": {
			golang:  int8(18),
			encoded: []byte{typeInt8, 0x12},
			tokens:  []Token{{A: TokenNumber, Data: int8(18)}},
		},
		"int16": {
			golang:  int16(16),
			encoded: []byte{typeInt16, 0x10, 0x0},
			tokens:  []Token{{A: TokenNumber, Data: int16(16)}},
		},
		"int32": {
			golang:  int32(32),
			encoded: []byte{typeInt32, 0x20, 0x0, 0x0, 0x0},
			tokens:  []Token{{A: TokenNumber, Data: int32(32)}},
		},
		"int64": {
			golang:  int64(64),
			encoded: []byte{typeInt64, 0x40, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
			tokens:  []Token{{A: TokenNumber, Data: int64(64)}},
		},

		"int_negative": {
			golang:  -5,
			encoded: []byte{typeInt64, 0xfb, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
			tokens:  []Token{{A: TokenNumber, Data: int64(-5)}},
		},
		"int8_negative": {
			golang:  int8(-8),
			encoded: []byte{typeInt8, 0xf8},
			tokens:  []Token{{A: TokenNumber, Data: int8(-8)}},
		},
		"int16_negative": {
			golang:  int16(-16),
			encoded: []byte{typeInt16, 0xf0, 0xff},
			tokens:  []Token{{A: TokenNumber, Data: int16(-16)}},
		},
		"int32_negative": {
			golang:  int32(-32),
			encoded: []byte{typeInt32, 0xe0, 0xff, 0xff, 0xff},
			tokens:  []Token{{A: TokenNumber, Data: int32(-32)}},
		},
		"int64_negative": {
			golang:  int64(-64),
			encoded: []byte{typeInt64, 0xc0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
			tokens:  []Token{{A: TokenNumber, Data: int64(-64)}},
		},

		"uint": {
			golang:  uint(5),
			encoded: []byte{0xa5},
			tokens:  []Token{{A: TokenNumber, Data: 5}},
		},
		"uint8": {
			golang:  uint8(18),
			encoded: []byte{typeUint8, 0x12},
			tokens:  []Token{{A: TokenNumber, Data: uint8(18)}},
		},
		"uint16": {
			golang:  uint16(16),
			encoded: []byte{typeUint16, 0x10, 0x0},
			tokens:  []Token{{A: TokenNumber, Data: uint16(16)}},
		},
		"uint32": {
			golang:  uint32(32),
			encoded: []byte{typeUint32, 0x20, 0x0, 0x0, 0x0},
			tokens:  []Token{{A: TokenNumber, Data: uint32(32)}},
		},
		"uint64": {
			golang:  uint64(64),
			encoded: []byte{typeUint64, 0x40, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
			tokens:  []Token{{A: TokenNumber, Data: uint64(64)}},
		},

		"float32": {
			golang:  float32(32.5),
			encoded: []byte{typeFloat32, 0x0, 0x0, 0x2, 0x42},
			tokens:  []Token{{A: TokenNumber, Data: float32(32.5)}},
		},

		"float64": {
			golang:  float64(64.5),
			encoded: []byte{typeFloat64, 0x0, 0x0, 0x0, 0x0, 0x0, 0x20, 0x50, 0x40},
			tokens:  []Token{{A: TokenNumber, Data: float64(64.5)}},
		},

		"float32_negative": {
			golang:  float32(-32.5),
			encoded: []byte{typeFloat32, 0x0, 0x0, 0x2, 0xc2},
			tokens:  []Token{{A: TokenNumber, Data: float32(-32.5)}},
		},

		"float64_negative": {
			golang:  float64(-64.5),
			encoded: []byte{typeFloat64, 0x0, 0x0, 0x0, 0x0, 0x0, 0x20, 0x50, 0xc0},
			tokens:  []Token{{A: TokenNumber, Data: float64(-64.5)}},
		},

		"slice": {
			golang:  []interface{}{testString, true},
			encoded: []byte{listStart, 0x74, 0x65, 0x73, 0x74, stringEnd, boolTrue, listEnd},
			tokens:  []Token{{A: tokenListStart}, {A: TokenString, Data: testString}, {A: tokenTrue}, {A: tokenListEnd}},
		},
		"byte_slice": {
			golang:  []byte{0x1, 0x30},
			encoded: []byte{typedArray, typeUint8, 2, 0x1, 0x30},
			tokens:  []Token{{A: tokenListStart}, {A: TokenNumber, Data: 0x1}, {A: TokenNumber, Data: 0x30}, {A: tokenListEnd}},
		},

		"array": {
			golang:  [2]bool{false, true},
			encoded: []byte{listStart, boolFalse, boolTrue, listEnd},
			tokens:  []Token{{A: tokenListStart}, {A: tokenFalse}, {A: tokenTrue}, {A: tokenListEnd}},
		},
		"map": {
			golang:  map[string]string{"a": "b"},
			encoded: []byte{dictStart, 0x61, stringEnd, 0x62, stringEnd, dictEnd},
			tokens:  []Token{{A: tokenDictStart}, {A: TokenString, Data: "a"}, {A: TokenString, Data: "b"}, {A: tokenDictEnd}},
		},
	}
)
